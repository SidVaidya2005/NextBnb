...
- Signature pieces still visual-only: `GuestFavoriteBadge`, `ProductTabs` (Homes/Experiences/Services), `CityLinkGrid`. `SearchBarPill` is now a full search popover (composes `DateRangeCalendar` / `DestinationPicker` / `GuestSelector`) that navigates to `/listings?where=`, `HeartButton` is wired to the wishlist via `useWishlist`, and `RatingDisplay` is fed real server aggregates.
